...
// === 修正後的儒略日（JD）公式（標準天文公式） ===
function julianDay(year, month, day, hour = 0, minute = 0) {
    const a = Math.floor((14 - month) / 12);
    const y = year + 4800 - a;
    const m = month + 12 * a - 3;
    let jd = day + Math.floor((153 * m + 2) / 5) + 365 * y + Math.floor(y / 4) - Math.floor(y / 100) + Math.floor(y / 400) - 32045;
    jd += (hour - 12) / 24.0 + minute / 1440.0;
    return jd;
}

function findSolarTermJD(year, targetLong) {
    let low = julianDay(year, 1, 1) - 20;
    let high = julianDay(year + 1, 1, 1) + 20;
    for (let i = 0; i < 80; i++) {   // 增加迭代次數確保精度
        const mid = (low + high) / 2;
        const L = solarLongitude(mid);
        let diff = (L - targetLong + 360) % 360;
...
function generateSolarTerms(year) {
    const solarTermLongitudes = [285,300,315,330,345,0,15,30,45,60,75,90,105,120,135,150,165,180,195,210,225,240,255,270];
    const terms = [];
    for (let i = 0; i < 24; i++) {
...
window.getBazi = function(year, month, day, hour = 0, minute = 0) {
    try {
        ...
        console.error("getBazi 內部錯誤:", e);   // 方便你在 F12 看到真正錯誤
...
window.getSolarTerms = function(year) {
    try {
        ...
        console.error("getSolarTerms 內部錯誤:", e);
        return { error: '節氣計算錯誤' };
    }
};
