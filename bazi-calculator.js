...
const lunarInfo = [ /* 完全保留你原本的 lunarInfo 陣列 */ 
...
function getLunarYearDays(year) { /* 完全保留你原本的所有 helper 函數 */ 
    let sum = 348;
...
function getLunar(sy, sm, sd) { /* 完全保留原本 getLunar */ 
...
window.getBazi = function(year, month, day, hour = 0, minute = 0) {
    try {
        ...
        return {
            ...
            pillars: {
                year: stems[yearstem] + BranchesNamesshort[yearbranch],
                month: stems[monthstem] + BranchesNamesshort[monthbranch],
                day: stems[daystem] + BranchesNamesshort[daybranch],
                hour: stems[hourstem] + BranchesNamesshort[hourbranch]
            },
            ...
            lunar: {
                year: lunarYearStr,
                month: isLeapStr + lunarMonthStr,
                day: lunarDayStr,
                full: `${lunarYearStr}年　${isLeapStr}${lunarMonthStr}月　${lunarDayStr}`,
                isLeap: lunar.isLeap
            },
